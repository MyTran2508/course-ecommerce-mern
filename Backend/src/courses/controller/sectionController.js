...
const {PATH_COURSE_LECTURE, PATH_COURSE_DOCUMENT} = require('../utils/awsS3Constant')
...
const uploadFileSection = asyncHandler(async (req, res) => {
    ...
    if(listError.length > 0) {
        const response = ResponseMapper.toDataResponse(listError, StatusCode.DATA_NOT_MAP, StatusMessage.DATA_NOT_MAP);
        res.json(response)
    } else {
